...
{ 
	...
	@try {
		if(receivedData == nil || [receivedData length]== 0)
			return;
		NSString *data = [[[NSString alloc] initWithData:receivedData encoding:NSASCIIStringEncoding] autorelease];
...
-(NSURLRequest *)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)redirectResponse
{
	...
	NSMutableURLRequest *newR =  [[NSMutableURLRequest alloc] init];
	int port =  [[ [request URL] port ]intValue];
	if(port == 0 )
		port = 80; 
	if([[request URL] query])
		 [newR setURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%d%@?%@",  [ [request URL] host ], port ,[[request URL] path ],[[request URL] query] ]]];
	else
		 [newR setURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%d%@", [ [request URL] host ],port,[[request URL] path ]]]];; 
	...
	[newR setValue: APPID  forHTTPHeaderField:@"ap-appid"];
